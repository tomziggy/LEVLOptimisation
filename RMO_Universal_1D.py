...
gpmeanprior = "max"
Variable1 = "Location of Maximum Camber (x/c)"
Variable2 = "Maximum Thickness"
...
lowerBound = [0.01] #Lower Limits of Variable1 and Variable2
upperBound = [0.25] #Upper Limits of Variable1 and Variable2
...
ca = setcamber*np.ones(len(features))
lc = camberlocation*np.ones(len(features))
features = np.hstack([ca.reshape(-1, 1),lc.reshape(-1, 1),features])
...
globalGP = GPTrain(features[:, 2].reshape(-1, 1), scalarised, meanPrior=gpmeanprior)
best_idx = np.argmax(scalarised)
best_x = features[best_idx]
best_y = scalarised[best_idx]

bounds = list(zip(lowerBound, upperBound))  # or [(0.025, 0.25)]
x_range = np.linspace(lowerBound, upperBound, 100).reshape(-1, 1)

# === GP Prediction and EI Evaluation
ei = expectedImprovement(x_range, globalGP, best_y, epsilon)
ei_history = []
rangePredictions, std_dev = BOGPEval(globalGP, x_range)

# === Plot GP surrogate with confidence interval
fig = plt.figure(figsize=(10, 6))
plt.plot(x_range, rangePredictions, label='GP Mean Prediction', color='blue')
plt.fill_between(x_range.ravel(),
                 rangePredictions - 2*std_dev,
                 rangePredictions + 2*std_dev,
                 color='blue', alpha=0.2, label='±2σ')
plt.scatter(features[:, 2], scalarised, c='red', label='Training Points')
plt.xlabel("Thickness")
plt.ylabel("Scalarised Objective")
plt.title("1D GP Surrogate Over Thickness")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(f"{destpng}/GP_1D_Surrogate.png")
...
plt.xlabel("Thickness")
plt.ylabel("EI")
plt.title("Expected Improvement Over Thickness")
plt.legend()
plt.grid(True)
plt.tight_layout()
...
while iteration < 50 and (iteration < 5 or np.max(ei) > 1e-7):
    ...
    scalarised = MTCHScalarisation(targets,alpha)
    scalarised = (scalarised - np.mean(scalarised)) / np.std(scalarised)
    globalGP = GPTrain(features[:, 2], scalarised, meanPrior=gpmeanprior)
    ...
    x_add = np.array([[setcamber, camberlocation, new_x[0]]])
    ...
    # === Plot GP Prediction over Thickness
    ...
    plt.ylabel("Predicted Scalarised Objective")
    plt.title("1D GP Surrogate Over Thickness")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    # === Plot Expected Improvement
    ei_values = expectedImprovement(x_range, globalGP, best_y, epsilon)

    fig = plt.figure(figsize=(12, 10))
    plt.plot(x_range, ei_values, color='green', label='Expected Improvement')
    plt.scatter(features[:, 2], np.zeros_like(features[:, 2]), c='red', marker='x', label='Sampled Thicknesses')
    plt.xlabel("Maximum Thickness")
    plt.ylabel("EI")
    plt.title(f"Expected Improvement at Iteration {iteration}")
    plt.legend()
