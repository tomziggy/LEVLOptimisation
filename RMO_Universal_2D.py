...
features = np.hstack([ca.reshape(-1, 1),features])
...
x_range = np.linspace(lowerBound[0], upperBound[0], 100)
y_range = np.linspace(lowerBound[1], upperBound[1], 100)
...
plt.legend()
plt.savefig(f"{destpng}/iteration0.png")
...
while iteration < 100 and (iteration < 10 or np.max(ei) > 1e-7):
    ...
    def build_feature_condition_matrix(design, conditions):
        return np.array([[setcamber, design[0], design[1], cond[0], cond[1]] for cond in conditions])
    ...
    x_add = np.array([[setcamber, new_x[0], new_x[1]]])
