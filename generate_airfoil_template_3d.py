...
def create_airfoil_and_fluid_domain(airfoil_coords, a:float):
    """
    ...
    vr = geompy.MakeVector(p1, p2)
    
    # rotate the given object around the given axis by the given angle
    rotated_airfoil = geompy.MakeRotation(airfoil_extrusion, vr, a*(-np.pi/180))
    ...
    geompy.addToStudy(rotated_airfoil, f"Airfoil Rotated")

    geompy.ExportSTL(rotated_airfoil, "airfoil.stl")
...
airfoil_coords = generate_naca_airfoil(m=f, p=l, t=h)
print("Coords Generated")
create_airfoil_and_fluid_domain(airfoil_coords, a)
