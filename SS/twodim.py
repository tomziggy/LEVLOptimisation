...
def blackbox(feature):
    camber, camber_loc = feature

    def create_airfoil(camber, camber_loc): #function to write Allrun File
    ...
    print(f"Camber = {cam} Location = {camber_loc}")
    create_airfoil(cam,loc)
...
lowerBound = [0, 0.3] # The lower bound of the input data is set to -10.
upperBound = [0.2, 0.8] # The upper bound of the input data is set to 10.

initialSampleSize = 5 # The initial sample size is set.
print("Setting Bounds")

sampler = qmc.LatinHypercube(d=2) # LHS initialised and number of dimensions is set.
...
for idx,i in enumerate(features):
    ix = idx+1
    print(f"Sample Simulation {ix}/{len(features)}. Max Camber = {i[0]} Location = {i[1]}")
...
#print("Printing 3D GP Model")
#fig = plt.figure(figsize=(10, 6))
#ax = fig.add_subplot(111, projection='3d')
#ax.scatter(features[:, 0], features[:, 1], targets, color='red', label='Samples')
#ax.plot_trisurf(features[:, 0], features[:, 1], y_pred, color='blue', alpha=0.7)
#ax.set_xlabel('Max Camber')
#ax.set_ylabel('Camber Location')
#ax.set_zlabel('Black Box Output')
#ax.set_title('Black Box Function with Gaussian Process Surrogate Model')
#destgp = os.path.join(destpng, "3d_gpmodel.png")
...
#print("Printing 3D Expected Improvement")
#ei = expectedImprovement(x_range, globalGP, best_y, 0.1)
#fig = plt.figure(figsize=(10, 6))
#ax = fig.add_subplot(111, projection='3d')
#ax.scatter(x_range[:, 0], x_range[:, 1], ei, color='green', label='Expected Improvement')
#ax.plot_trisurf(x_range[:, 0], x_range[:, 1], ei, color='green', alpha=0.7)
#ax.set_xlabel('Max Camber')
#ax.set_ylabel('Camber Location')
#ax.set_zlabel('Expected Improvement')
#ax.set_title('Expected Improvement Surface')
#destei1 = os.path.join(destpng, "3d_expectedimprovement.png")
#plt.savefig(destei1, dpi=300)



nested_bounds = [(lowerBound, upperBound)] # The bounds for each dimension are set to the lower and upper bounds.
bounds = [tuple(bound) for bound in nested_bounds[0]]

print("bounds=",bounds)
result = optimize_ei(bounds, globalGP, best_y, 1) # The expected improvement is optimized using differential evolution.
...
#print("Printing 3D Expected Improvement with Optimal Feature")
ei = expectedImprovement(x_range, globalGP, best_y, 0.1)
#fig = plt.figure(figsize=(10, 6))
#ax = fig.add_subplot(111, projection='3d')
#ax.scatter(features[:, 0], features[:, 1], targets, color='blue', label='Previous Samples')
#ax.scatter(x_range[:, 0], x_range[:, 1], ei, color='green', label='Expected Improvement')
#ax.scatter(optimal_feature[0], optimal_feature[1], max_ei, color='red', label='Optimal Feature')
#ax.set_xlabel('Max Camber')
#ax.set_ylabel('Camber Location')
#ax.set_zlabel('Expected Improvement / Black Box Output')
#ax.set_title('Expected Improvement with Optimal Feature')
#destei2 = os.path.join(destpng, "4d_expectedimprovement_with_optimal_from_samples.png")
...
while np.max(ei) > 1e-5:
    ...
    print("Current Best It:",best_idx,"Best Camber & Location",best_x,"Best L/D",best_y)
    ...
    # x_range = np.array(x_range).squeeze()
    # if x_range.ndim == 1:
    #     x_range = x_range.reshape(1, -1)
    
    # X, Y = np.meshgrid(x_range[:, 0], x_range[:, 1])

    # Z = np.interp(X.flatten(), x_range[:, 0], y_pred).reshape(X.shape)

    # ax1.plot_surface(X, Y, Z, cmap='viridis', alpha=0.7)
    
    # print(features.shape)
    # print(targets.shape)
    # ax1.scatter(features[:, 0], features[:, 1], targets, color='red', label="Sampled Points")
    
    # ax1.set_title(f"Iteration #{iteration} - GP Model Surface")
    # ax1.set_xlabel("Feature 1")
    # ax1.set_ylabel("Feature 2")
    # ax1.set_zlabel("Predicted Value")
    # ax1.legend()

    # ax2 = fig.add_subplot(2, 1, 2, projection='3d')
    # Z_ei = np.interp(X.flatten(), x_range[:, 0], ei).reshape(X.shape)

    # ax2.plot_surface(X, Y, Z_ei, cmap='plasma', alpha=0.7)
    # ax2.scatter(new_x[0], new_x[1], max_ei, color='green', label="New Sampled Point")
    # ax2.set_title(f"Iteration #{iteration} - Expected Improvement")
